...
targeted_list = "、".join(list(os.listdir(os.path.join(path, "Targeted")))).replace('、packetbeat','')
...
parser.add_argument("-T", "--Targeted", default="httpd",
                    type=str, help=f"請選擇靶機image名稱，有[{targeted_list}]")
...
try:
    if args.Targeted not in targeted_list.split('、'):
        raise Exception(f"Targeted '{args.Targeted}' 不存在，請重新選擇{targeted_list}")
    ...
    with open(os.path.join(path, 'docker-compose.yml'), 'r') as f:
        ...
        yml = re.sub(machines[0], args.Targeted, yml)
    ...
    with open(os.path.join(path, 'makefile'), 'r') as f:
        ...
        for _ in result:
            machines.append(_.groups()[0])
        makefile = re.sub(machines[0], args.Targeted, makefile)
    ...
    with open(os.path.join(path, 'makefile'), 'w') as f:
        f.write(makefile)
    print(f"攻防環境模組已設置完成，攻擊機: {args.Targeted} ，靶機: {args.Attacker} ")
